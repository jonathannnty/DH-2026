...
describe('Level 1: Agent Service Direct Testing', () => {
  describe('Health & Registration', () => {
    ...
    it('list agents returns detailed agent information', async () => {
      ...
      data.agents.forEach((agent) => {
        ...
        expect(['idle', 'running', 'completed', 'error']).toContain(agent.status);
      });
    });
  });
  ...
  describe('Agent Execution Pipeline', () => {
    ...
    it('triggers analysis pipeline', async () => {
      const { status, data } = await request<{ success: boolean; status: string }>(
        'POST',
        '/analyze',
        {
          sessionId,
    ...
    it('each agent produces structured output', async () => {
      ...
      const profileAnalysis = results.profile_analysis as Record<string, unknown>;
  ...
  describe('Error Handling', () => {
    ...
    it('returns 404 for unknown session', async () => {
      const { status } = await request('GET', '/status/nonexistent-session-12345');
