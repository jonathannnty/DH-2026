...
const RUN_LEVEL3_E2E_TESTS = process.env.RUN_LEVEL3_E2E_TESTS === "true";
...
describe.skipIf(!RUN_LEVEL3_E2E_TESTS)(
  "Level 3: End-to-End Frontend Testing",
  () => {
    ...
    describe("Onboarding Questions Flow", () => {
      it("displays professional onboarding questions", async () => {
        ...
        const questions = screen.queryAllByText(
          /question|select|what|describe/i,
        );
      ...
      it("allows user to answer questions sequentially", async () => {
        ...
        await waitFor(() => {
          expect(
            screen.queryByText(/next|continue|proceed/i),
          ).toBeInTheDocument();
        });
      });
      ...
      it("collects profile data through all questions", async () => {
        ...
        expect(
          screen.queryByText(/progress|question|step/i),
        ).toBeInTheDocument();
      });
    ...
    describe("Results Display", () => {
      it("displays results page after analysis completes", async () => {
        ...
        const titleOrHeading = screen.queryByText(
          /career|recommendation|result/i,
        );
      ...
      it("shows career recommendations", async () => {
        ...
        const recommendations = screen.queryByText(
          /career path|opportunity|role/i,
        );
      ...
      it("displays actionable next steps", async () => {
        ...
        const nextSteps = screen.queryByText(
          /next step|action|do next|recommendation/i,
        );
      ...
      it("disables form submission during analysis", async () => {
        ...
        if (submitBtn) {
          ...
          await waitFor(() => {
            ...
            const hasDisabled = allBtns.some((btn) =>
