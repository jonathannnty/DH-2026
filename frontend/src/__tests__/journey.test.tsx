...
describe('Home', () => {
  ...
  it('shows error message when session creation fails', async () => {
    server.use(
      http.post('http://localhost:3001/sessions', () =>
...
describe('Onboarding', () => {
  ...
  it('loads session and shows greeting message', async () => {
    renderAt(
      `/onboarding?session=${SESSION_INTAKE.id}`,
      <Onboarding />,
    );

    await waitFor(() => {
      expect(screen.getByText(/Let's discover your ideal career path/)).toBeInTheDocument();
    });
  ...
  it('shows error state when session fetch fails', async () => {
    ...
    renderAt(
      '/onboarding?session=does-not-exist',
      <Onboarding />,
    );

    await waitFor(() => {
      expect(screen.getByText(/Session not found or unavailable/)).toBeInTheDocument();
    });
    expect(screen.getByRole('link', { name: /Start a new assessment/ })).toBeInTheDocument();
  });
...
describe('Results', () => {
  ...
  it('shows rec count in the heading', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Your Top 2 Career Matches/)).toBeInTheDocument();
    }, { timeout: 3000 });
  });
