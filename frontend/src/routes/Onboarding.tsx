...
export default function Onboarding() {
  ...
  useEffect(() => {
    ...
    getSession(sessionId)
      .then((s) => {
        ...
        if (lastMsg?.content.includes('profile is ready for analysis')) {
  ...
  if (!sessionId) {
    return (
      <div style={{ ...wrap, alignItems: 'center', justifyContent: 'center', gap: 12, flex: 1 }}>
    ...
    return (
      <div style={wrap}>
        ...
        <div style={inputBar}>
          ...
          <button style={sendBtnStyle(true)} disabled>Send</button>
        </div>
      </div>
    );
  }
  ...
  if (loadState === 'error') {
    return (
      <div style={{ ...wrap, alignItems: 'center', justifyContent: 'center', gap: 16, flex: 1 }}>
        <p style={{ fontWeight: 600 }}>Session not found or unavailable.</p>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', maxWidth: 320, textAlign: 'center' }}>
  ...
  return (
    <div style={wrap}>
      ...
      {intakeComplete ? (
        <div style={{ ...inputBar, flexDirection: 'column', alignItems: 'center', gap: 8, paddingBottom: 24 }}>
          ...
          {analyzeError && (
            <p style={{ color: 'var(--danger)', fontSize: '0.825rem' }}>{analyzeError}</p>
          )}
        </div>
      ) : (
        <div style={inputBar}>
